...
ConnectionsManager& ConnectionsManager::getInstance(int32_t instanceNum) {
    /*static ConnectionsManager *cg_instances[MAX_ACCOUNT_COUNT];

    if (cg_instances[instanceNum] == nullptr) {
        cg_instances[instanceNum] = new ConnectionsManager(instanceNum);
    }

    return *cg_instances[instanceNum];*/
    switch (instanceNum) {
        case 0:
...
void ConnectionsManager::processServerResponse(TLObject *message, int64_t messageId, int32_t messageSeqNo, int64_t messageSalt, Connection *connection, int64_t innerMsgId, int64_t containerMessageId) {
    ...
    int64_t timeMessage = (int64_t) ((messageId != 0 ? messageId : innerMsgId) / 4294967296.0 * 1000);
